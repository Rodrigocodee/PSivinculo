      ...
      <header className="sticky top-0 z-50 border-b border-white/55 bg-background/78 backdrop-blur-xl supports-[backdrop-filter]:bg-background/72">
        <div className="container mx-auto px-4 py-3">
          ...
          <Link to="/" className="flex min-w-0 items-center gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[1.1rem] gradient-primary shadow-[0_18px_40px_-20px_hsl(220_65%_60%/0.72)] ring-1 ring-white/55 sm:h-11 sm:w-11">
              <Brain className="h-5 w-5 text-primary-foreground" />
            </div>
            <div className="min-w-0">
              <span className="block truncate font-heading text-lg font-bold tracking-[-0.03em] text-foreground sm:text-xl">Psivinculo</span>
          ...
          <Link
            to="/"
            className="shrink-0 rounded-full gradient-primary px-3 py-2 text-xs font-semibold text-primary-foreground shadow-[0_18px_44px_-24px_hsl(220_65%_60%/0.72)] transition-all hover:-translate-y-0.5 hover:opacity-95 sm:px-5 sm:py-2.5 sm:text-sm"
          >
            Voltar ao inicio
          </Link>
          </div>

          <nav className="-mx-4 mt-3 flex gap-2 overflow-x-auto px-4 pb-1 md:hidden" aria-label="Navegacao institucional">
            {publicHeaderLinks.map((link) => renderHeaderLink(link))}
          </nav>
        </div>
      </header>

      <main className="px-4 pb-16 pt-8 sm:pb-24 sm:pt-12 lg:pt-16">
        <div className="container mx-auto max-w-5xl">
          <section className="relative overflow-hidden rounded-[1.5rem] border border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.92),rgba(244,247,252,0.86))] shadow-[0_30px_90px_-44px_rgba(49,61,114,0.34)] backdrop-blur-xl sm:rounded-[2.25rem]">
            ...
            <div className="relative px-5 py-8 sm:px-10 sm:py-12 lg:px-12">
              ...
              <h1 className="mt-6 font-heading text-3xl font-bold leading-tight text-foreground sm:text-4xl md:text-5xl">
                {title}
              </h1>
              <p className="mt-5 max-w-3xl text-base leading-7 text-foreground/68 sm:text-lg sm:leading-8">
                {description}
              </p>
            </div>
          </section>
