...
export type PsychologistNotification = {
  id: string;
  type: string;
  title: string;
  message: string;
...
export async function listPsychologistNotifications(): Promise<PsychologistNotification[]> {
  ...
  const { data, error } = await supabase
    ...
    .eq("usuario_id_destino", scope.userId)
  ...
  return ((data ?? []) as NotificationRow[]).map((notification) => {
    ...
    return {
      ...
      timestamp,
    } satisfies PsychologistNotification;
  });
}

export async function markPsychologistNotificationsAsRead(notificationIds: string[]) {
